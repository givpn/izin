...
surat=$(curl -sS https://raw.githubusercontent.com/givpn/permission/master/ip | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/givpn/izin/bot/botpanel/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
